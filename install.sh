...
./configure --user=daemon --group=daemon --prefix=/usr/local/openresty/nginx/ --with-http_stub_status_module --with-http_sub_module --with-http_gzip_static_module --without-mail_pop3_module --without-mail_imap_module --without-mail_smtp_module  --add-module=../ngx_devel_kit-0.2.17rc2/ --add-module=../lua-nginx-module-0.7.4/
make -j8
make install 
#rm -rf /data/src
...
unzip master.zip
mv ngx_lua_waf-master/* /usr/local/openresty/nginx/conf/
